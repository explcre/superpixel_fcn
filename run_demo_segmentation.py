...
def superpixel_covered_area(mask_image, superpixel_mask, threshold):
    ...
    for spixel_id in unique_spixels:
        area_covered = np.sum((superpixel_mask == spixel_id) & (mask_image == 1))
        area_spixel = np.sum(superpixel_mask == spixel_id)

        if area_covered / area_spixel >= threshold:
...
def visualize_selected_superpixels(img, superpixel_mask, final_set):
    ...
    img_selected = img * np.repeat(mask_selected[:, :, np.newaxis], 3, axis=2)
    return img_selected
